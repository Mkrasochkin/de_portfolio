...
def foreach_batch_function(df, epoch_id):
    ...
    feedback_df = df.withColumn("fedback", lit(None).cast(StringType()))
    feedback_df.write \
        .format("jdbc") \
        .option("url", 'jdbc:postgresql://rc1a-fswjkpli01zafgjm.mdb.yandexcloud.net:5432/de') \
    ...
    kafka_df = df.withColumn("value", to_json(struct([col(c) for c in df.columns if c != "feedback"])))
    # отправляем сообщения в результирующий топик Kafka без поля feedback
    kafka_df.select("value").write \
        .format("kafka") \
        .option("kafka.bootstrap.servers", "rc1b-2erh7b35n4j4v869.mdb.yandexcloud.net:9091") \
        .option("topic", "student.topic.cohort7.mkrasochkin.out") \
        .save()
...
restaurant_read_stream_df = spark.readStream \
    ...
    .option('subscribe', 'student.topic.cohort7.mkrasochkin') \
    .load()
...
filtered_read_stream_df = restaurant_read_stream_df \
    .select(from_json(col("value").cast("string"), incomming_message_schema).alias("json")) \
    .select("json.*") \
